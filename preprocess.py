...
def modeRD(RD):
    ...
    count = np.bincount(newRD,minlength=50)
...
def Read_seg_file(num_col, num_bin,segFile):
    """
...
def preProcessData(refpath,bam,segNumber):

    col = 50
    k = 10

    path = os.path.abspath('.')
    segpath = path + str("/seg")+str(segNumber)
    ...
    reference = refpath + '/chr' + str(21) + '.fa'
    ref = read_ref_file(reference, 21, ref)
    chrLen = np.full(23, 0)
    for i in range(1, 23):
        chrLen[i] = len(ref[i])
    ...
    # return all_start,all_end,all_rd,splitBinStart,splitBinEnd,splitBinRD,mode,newDataSet_OCSVM_RD
    return all_chr,all_start,all_end,all_RD,mode
